...
namespace OfficeOpenXml.Utils.CompundDocument
{
    ...
    internal class CompoundDocumentFile : IDisposable
    {
        ...
        private static readonly byte[] header = new byte[] { 0xD0, 0xCF, 0x11, 0xE0, 0xA1, 0xB1, 0x1A, 0xE1 };
        ...
        // (4 bytes): This integer field contains the starting sector number for the DIFAT.
        private int _numberofDIFATSectors;

        private int _numberOfFATSectors;

        // (4 bytes): This integer field MUST be set to 0x00001000. This field specifies the maximum size of a user-defined data stream that is allocated from the mini FAT and mini stream, and that cutoff is 4,096 bytes.Any user-defined data stream that is larger than or equal to this cutoff size must be allocated as normal sectors from the FAT.
        // (4 bytes): This integer field contains the starting sector number for the mini FAT.
        ...
        // (4 bytes): This integer field contains the count of the number of mini FAT sectors in the compound file.
        // (4 bytes): This integer field contains the count of the number of DIFAT sectors in the compound file.
        private int _sectorSize, _miniSectorSize;

        private int _sectorSizeInt;

        // (4 bytes): This integer field contains the count of the number of FAT sectors in the compound file.
        // (4 bytes): This integer field contains the starting sector number for the directory stream.
        ...
        private short sectorShif, minSectorShift;
        ...
        // (4 bytes): This integer field MAY contain a sequence number that is incremented every time the compound file is saved by an implementation that supports file transactions.This is the field that MUST be set to all zeroes if file transactions are not implemented.<1>
